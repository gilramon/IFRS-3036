...
namespace fftbench
{
    class Program
    {
        ...
        static async Task Run(int size, int repeat)
        {
            ...
            Console.WriteLine("FFT size: " + Util.Pow(2, size));
        ...
        {
            int k = 0, nargs = args.Length;

            if (nargs > 0 && args[0] == "fftbench")
            {
                k = 1;
            }
            ...
            if (args.Length > k)
            {
                if (!int.TryParse(args[k], out size))
                {
                    return false;
                }
            }
            ...
            if (args.Length > k + 1)
            {
                if (!int.TryParse(args[k + 1], out repeat))
                {
                    return false;
                }
            }
